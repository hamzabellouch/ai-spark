...
function clearHistory() {
    showConfirmAlert(translations[currentLang].alertWarning, "هل أنت متأكد من مسح المحادثة بالكامل؟", async () => {
        try {
...
async function sendPrompt() {
    ...
    const synthesize = toggleSynthesis.checked;
    
    // Show and reset progress container
    progressContainer.classList.remove("hidden");
    setStepState(stepBroadcast, "loading", translations[currentLang].stepBroadcastLoading);
    setStepState(stepCollect, "pending", translations[currentLang].stepCollectPending);
    ...
    setTimeout(() => {
        setStepState(stepBroadcast, "completed", translations[currentLang].stepBroadcastCompleted);
        setStepState(stepCollect, "loading", translations[currentLang].stepCollectLoading);
    }, 4000);
    ...
    try {
        ...
        setStepState(stepCollect, "completed", translations[currentLang].stepCollectCompleted);
...
// Global pointermove handler for premium HoverButton glowing trail animation
(function() {
    let hoverButtonLastAdded = 0;
    ...
    document.addEventListener("pointermove", (e) => {
        ...
        if (currentTime - hoverButtonLastAdded > 100) {
            hoverButtonLastAdded = currentTime;
            
            const rect = button.getBoundingClientRect();
            const x = e.clientX - rect.left;
            const y = e.clientY - rect.top;
            
            createCircle(button, x, y);
        }
    });

    function createCircle(button, x, y) {
        const buttonWidth = button.offsetWidth || 0;
        const xPos = x / buttonWidth;
        
        const circle = document.createElement("div");
        circle.className = "hover-button-circle";
        
        // Dynamically compute the linear gradient color mapping matching the React component:
        // color = linear-gradient(to right, var(--circle-start) xPos*100%, var(--circle-end) xPos*100%)
        const startColor = getComputedStyle(button).getPropertyValue("--circle-start").trim() || "#a0d9f8";
        const endColor = getComputedStyle(button).getPropertyValue("--circle-end").trim() || "#3a5bbf";
        ...
        // After 1000ms transition to fade out
        setTimeout(() => {
            circle.classList.remove("fade-in");
            circle.classList.add("fade-out");
        }, 1000);
        
        // After 2200ms completely remove circle from DOM
        setTimeout(() => {
            circle.remove();
        }, 2200);
    }
