...
async def auto_launch_browser():
    ...
    try:
        ...
        await browser_ctrl.open_or_get_tab("broadcast", "http://127.0.0.1:8000")
...
async def launch_browser_tabs(request: Request, payload: LaunchPayload = Body(default=LaunchPayload())):
    ...
    try:
        ...
        browser_ctrl = None
        error_msg = f"يرجى إغلاق جميع نوافذ متصفح {current_browser_name.capitalize()} المفتوحة بالكامل أولاً (تأكد من إغلاقه تماماً من مدير المهام إذا لزم الأمر)، ثم حاول مجدداً لكي يستطيع البرنامج تشغيل متصفحك الحقيقي بوضع التحكم والاتصال بحساباتك الافتراضية المفتوحة مسبقاً. تفاصيل الخطأ: {e}"
...
def select_browser_cli():
    ...
    print("==================================================================")
    try:
        choice = input("Enter choice (1-6) [Default: 3 (Brave)]: ").strip()
