...
@app.get("/favicon.ico")
async def favicon():
    return FileResponse("static/favicon.ico") if os.path.exists("static/favicon.ico") else HTMLResponse(status_code=204)
...
async def launch_browser_tabs(request: Request, payload: LaunchPayload = Body(default=LaunchPayload())):
    ...
    try:
        ...
        return {"status": "success", "message": f"{current_browser_name.capitalize()} launched with AI tabs."}
        
    except Exception as e:
        browser_ctrl = None
        error_msg = (
            f"تعذر تشغيل المتصفح أو الاتصال به. يرجى التحقق مما يلي:\n"
            f"1. تأكد من إغلاق جميع نوافذ متصفح {current_browser_name.capitalize()} المفتوحة تماماً (من مدير المهام Task Manager إذا لزم الأمر) لكي يتمكن البرنامج من تحرير القفل والتحكم بملف تعريفك الافتراضي.\n"
            f"2. إذا كنت تستخدم Google Chrome (إصدار 136 فما فوق)، فإن جوجل تحظر التحكم عن بُعد في ملف التعريف الافتراضي لأسباب أمنية. في هذه الحالة، ننصحك باستخدام متصفح بديل مثل Brave أو Edge أو Firefox.\n"
            f"تفاصيل الخطأ: {e}"
        )
