...
3. **Run the script**:
   ...
   ./run_linux.sh
   ```
   - The script will automatically search for a working Python installation on your system (either `python3` or `python`), verify execution capability (Python 3.11+ is required), initialize a local Python virtual environment (`.venv`), install dependencies, and run the backend.
   - It also detects Snap/Flatpak browser installations and automatically configures a local fallback profile directory under `profiles/` if standard system paths are unavailable.
