...
3. **Double-click `run_macos.command`** or run `./run_macos.command` in terminal to start the local backend server.
   - The script will automatically search for a working Python installation on your system (either `python3` or `python`), verify execution capability (Python 3.11+ is required), initialize a local Python virtual environment (`.venv_macos`), and install dependencies.
