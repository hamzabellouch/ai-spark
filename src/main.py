...
async def launch_browser_tabs(request: Request, payload: LaunchPayload = Body(default=LaunchPayload())):
    ...
    try:
        ...
        error_msg = (
            ...
            f"1. تأكد من إغلاق جميع نوافذ متصفح {current_browser_name.capitalize() if not os.path.isabs(current_browser_name) else 'browser'} المفتوحة تماماً (من مدير المهام Task Manager إذا لزم الأمر) لكي يتمكن البرنامج من تحرير القفل والتحكم بملف تعريفك الافتراضي.\n"
